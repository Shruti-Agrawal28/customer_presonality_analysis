...
from analysis.entity import config_entity, artifact_entity
...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self) -> artifact_entity.ModelEvaluationArtifact:
        try:
            ...
            y_pred = model.predict(test_df[input_feature_name])
            ...
            y_pred = current_model.predict(test_df[input_feature_name])
            y_true = target_df
            print(f"Prediction using trained model: {(y_pred[:5])}")
            current_model_score = accuracy_score(y_true, y_pred)
